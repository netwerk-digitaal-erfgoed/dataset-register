...
// the same `void:subset` that holds the `subject-uris-sampled` measurement, so
// the right namespace is selected even when the dataset has other subsets (e.g.
// IIIF); the reason-namespace FILTER isolates subject-resolution failures from
// any other failure usages on the activity. Returns [] until the DKG ships the
// data, on error, or when nothing failed.
