...
// the dataset’s subject namespace, as a `prov:Usage` carrying the sampled URI
// (`prov:entity`) and a typed reason concept. Two data shapes are read so the
// panel works across the DKG’s breaking switch to unified outcomes: the legacy
// `failure:reason` (`subject-resolution-failure#`, failures only) and the
// current `resolution:outcome` (`subject-resolution-outcome#`, every sampled URI
// – successes and failures). `parsePersistentUriFailureReason` keeps only the
// failure concepts, so the resolved/landing-page successes the new shape also
// enumerates are skipped.
//
// The forward path anchors on the `subject-uris-sampled` measurement
// specifically – not just any measurement on the subset – because that
// measurement and `subject-uris-resolved` are both generated by the *same*
// activity that holds the usages; matching either would bind the activity twice
// and duplicate every URI. Anchoring on the one sampled measurement (plus SELECT
// DISTINCT as a guard) keeps each URI once. Selecting via this subset also picks
// the right namespace when the dataset has other subsets (e.g. IIIF); the
// reason-namespace FILTER drops any non-resolution usages on the activity.
// Returns [] until the DKG ships the data, on error, or when nothing failed.
async function fetchPersistentUriFailures(
  ...
  const query = `
    ...
    PREFIX resolution: <https://def.nde.nl/resolution#>
    SELECT DISTINCT ?uri ?reason WHERE {
      ...
      ?usage prov:entity ?uri ;
        (failure:reason|resolution:outcome) ?reason .
      FILTER(
        STRSTARTS(STR(?reason), "${SUBJECT_RESOLUTION_FAILURE_BASE_URI}") ||
        STRSTARTS(STR(?reason), "${SUBJECT_RESOLUTION_OUTCOME_BASE_URI}")
      )
    }
  `;
  ...
  try {
    ...
    for await (const raw of bindingsStream) {
      ...
      const reason = parsePersistentUriFailureReason(binding.reason?.value);
...
export async function fetchDatasetDetail(
  ...
  // Cartesian-products multi-valued properties (theme, spatial, type, etc.).
  // On a dataset with 20 themes × 13 spatial × 9 types, that produced
