...
| [`dct:spatial`](http://purl.org/dc/terms/spatial)                                          | Spatial coverage. Only IRI references (e.g. GeoNames) are accepted; structured `dct:Location` nodes with `dcat:bbox` / `dcat:centroid` / `dcat:geometry` are not supported. | [`schema:spatialCoverage`](https://schema.org/spatialCoverage)                       |
| [`dct:temporal`](http://purl.org/dc/terms/temporal)                                        | Temporal coverage. ISO 8601 literals are normalized to a `dct:PeriodOfTime` blank node with `dcat:startDate` and `dcat:endDate`.                                            | [`schema:temporalCoverage`](https://schema.org/temporalCoverage)                     |
...
### DCAT-AP-NL alignment

The table below compares the schema.org input, the resulting DCAT term, the data-type
constraints, and the effective cardinality, calling out where the register narrows
[DCAT-AP-NL 3.0](https://docs.geostandaarden.nl/dcat/dcat-ap-nl30/) or relies on auto-fill at ingest.
